...
var Police = {
  ...
  checkToolDefinition: function(appParam) {
    ...
    if(!flexist) {
      console.log(ERR("Tool Parse Error:"), "Inputlist does not exist in "+tool.name+".");
      return false;
    }
    if(flempty) {
      console.log(ERR("Tool Parse Error:"), "Inputlist empty in "+tool.name+".");
      return false;
    }

    //must have a name, description and invoke command
    if(tool.codeobj.name == "" || tool.codeobj.description == "" || tool.codeobj.invoke == "") {
      console.log(ERR("Tool Parse Error:"), "Missing required definition in "+tool.name+".");
      return false;
    }
    ...
    if(!propnames) {
      console.log(ERR("Tool Parse Error:"), "Property names not valid in "+tool.name+".");
      return false;
    }

    //check if JSON is valid
    if(!tool.valid) {
      console.log(ERR("Tool Parse Error:"), "JSON syntax not valid in "+tool.name+".");
      return false;
    }
    ...
    if(looping){
      if(notlooping) {
        console.log(ERR("Tool Parse Error:"), "'l'A need to be in all LEASHs in "+tool.name+".");
        return false;
      }
      if(ltOne) {
        console.log(ERR("Tool Parse Error:"), "Looping arrangement only allow one LEASH in "+tool.name+".");
        return false;
      }
    }
    ...
    if(isLowerHierarchy) {
      console.log(ERR("Tool Parse Error:"), "You can't use output list from same or lower hierarchy in "+tool.name+", because they are not exist by that step.");
      return false;
    }
  ...
  checkLEASH: function(leashString) {
    ...
    if(depthBracket != 0) {
      console.log(ERR("LEASH Parse Error:"), "Open and close brackets don't match in expression: "+leashString+".");
      return false;
    }
    if(depthQuote % 2 != 0) {
      console.log(ERR("LEASH Parse Error:"), "Quotes don't match in expression: "+leashString+".");
      return false;
    }
    ...
    if(!legalSeg){
      console.log(ERR("LEASH Parse Error:"), "Illegal Segment indicator in LEASH expression: "+leashString+".");
      return false;
    }
    //Segment type
    if(!legalType){
      console.log(ERR("LEASH Parse Error:"), "Illegal Segment format in LEASH expression: "+leashString+".");
      return false;
    }
    //Segment ordering
    if(wrongOrder) {
      console.log(ERR("LEASH Parse Error:"), "Segments not correctly ordered in LEASH expression: "+leashString+".");
      return false;
    }
  ...
  checkObject: function(obj) {
    ...
    if(!legalKey){
      console.log(ERR("Object Parse Error:"), "Unrecognized key name in Object: "+JSON.stringify(obj)+".");
      return false;
    }
    //Segment type
    if(!legalType){
      console.log(ERR("Object Parse Error:"), "Illegal Segment format in Object: "+JSON.stringify(obj)+".");
      return false;
    }

    if(wrongOrder) {
      console.log(ERR("LEASH Parse Error:"), "Segments not correctly ordered in LEASH object: "+JSON.stringify(obj)+".");
      return false;
    }
